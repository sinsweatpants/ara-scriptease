import { createRulerElement } from './ruler';
...
function createPage(pageNo: number): PageRefs {
  ...
  const horizontalRuler = createRulerElement('horizontal');
  const verticalRuler = createRulerElement('vertical');
  ...
  page.appendChild(horizontalRuler);
  page.appendChild(verticalRuler);
